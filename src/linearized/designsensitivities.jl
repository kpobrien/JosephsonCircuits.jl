...
values at `p` (a scattering block is reported as zero in both the value
and its Jacobian row, since its dependence is carried separately by
[`designblockjacobian`](@ref)), and the complex `length(names)` by
`length(parameters)` Jacobian. Each perturbed parameter is taken as
`real(p[q])`. The builder must keep the circuit topology fixed as the
...
while a block constructed inside the builder is differentiated. Each
perturbed parameter is taken as `real(p[q])`. Returns a vector of
