...
[`NoiseCovariance`](@ref) is rejected here with an `ArgumentError`: it
gives the covariance outright rather than deriving it, and is not lowered
into a circuit.
"""
...
A block with a [`NoiseCovariance`](@ref) does not reach here: the noise
plan rejects it (see [`planscatteringnoise`](@ref)).
"""
