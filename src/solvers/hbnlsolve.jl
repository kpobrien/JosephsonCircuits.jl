...
rejected with an `ArgumentError`. See `src/circuit/mna.jl`.
...
function nonlinearsetup(w::NTuple{N,Float64}, sources::Vector{SourceTuple{N}},
    ...
    # See circuit/mna.jl for details.
    Rbnmout = Rbnm
    ...
    # are node conductances, which is algebraically identical to carrying
    # their currents and keeps the system smaller
    Nauxscattering = countscatteringports(psc)*Nmodes
    Naux = length(coupledbranches)*Nmodes + Nauxscattering
    ...
    elseif devicex
        ...
        devjunctions = junctionstructure(Float64, Amatrixindices,
            ...
            Nfreq, backend)
        cjp = planstructurecomplexjacobian(Jxt, devjunctions, invLnm, Gnm,
            Cnm, wmodesm, wmodes2m, backend; transposed = true)
    ...
    elseif isnothing(Jx)
        ...
        # the host plan is the same kind: the Josephson map just built plus
        # the linear term gathered once at these mode frequencies
        Jx, planstructurecomplexjacobian(Jx, complexjosephson.junctions,
    ...
    elseif needjr
        ...
        junctions = junctionstructure(Float64, Amatrixindicesaliased,
    ...
    canonwork = if hasdc
        ...
        br = Nauxscattering > 0 ?
            dcblockrows(stampedblocks, dcplan.componentof, Nmodes,
                dcplan.modeindex, Nnodes - 1, Lscale;
...
function addsources!(bbm, modes, sources, portindices, portnumbers,
    ...
    for source in sources
        ...
        # a direct current is real: the phase of a complex amplitude has no
        # meaning at zero frequency, and stamping its real part would drop
        # the imaginary part without a word
        if all(iszero, mode) && !iszero(imag(current))
            throw(ArgumentError(lazy"The direct current source at port $(port) has the complex amplitude $(current); a source at the zero frequency mode $(mode) must be real."))
        end
        ...
        if haskey(portdict, port)
            ...
            if haskey(modedict, mode)
                ...
                bbm[(edge2indexdict[key]-1)*Nmodes+modeindex] += Lscale*current/phi0
            else
