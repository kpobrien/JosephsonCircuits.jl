...
function modebandmask(Amatrixmodes::AbstractMatrix, p)
    shells = _shells(Amatrixmodes, p)
    return [_withinband(Amatrixmodes[i, j], p, shells)
...
not be singular: a node whose stiffness at some mode frequency lives in a
...
function refactorize!(pc::ModeCouplingPreconditioner)
    ...
    # crosses to the host
    cosfd = cosphimatrix(pc.sys)
    A = if pc.P isa BlockStructure
        # the block factorization assembles its own blocks from the same
        # coefficients
        BlockJacobian(pc.P, cosfd)
    else
        assemblerealjacobian!(pc.nzval, pc.deviceplan, cosfd)
