...
# the evaluation point of a problem without a direct current block: the
# harmonic state itself
function _movepoint!(p::HBNonlinearProblem, u::AbstractVector)
    setpoint!(p.sys, u)
    p.pointstamp[] += 1
    return p
end
...
struct JacobianOperator{P<:HBNonlinearProblem,U<:AbstractVector}
    prob::P
    u::U
    # the problem's point stamp when the point was last set to `u`
...
# the point is frozen at `u`: when something else (a residual evaluation,
# another operator) has moved the problem's point since, it is set back
# before the product, and not otherwise, so the products of a Krylov loop
# pay nothing for the check
function _synchronize!(J::JacobianOperator)
    J.stamp[] == J.prob.pointstamp[] && return J
    isaugmented(J.prob) ? _setcanonical!(J.prob, J.u) :
        _movepoint!(J.prob, J.u)
    J.stamp[] = J.prob.pointstamp[]
    return J
end
...
# the point was set at construction and is only set again if the problem
