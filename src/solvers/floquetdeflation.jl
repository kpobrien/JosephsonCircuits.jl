...
A deflation may instead build its coarse correction from `G = Z'*J*Z` and a
truncated pseudo inverse of it. That pairing is the natural one for a
symmetric positive definite system. This Jacobian is none of those things: it is nonsymmetric,
...
Per application: one base solve and two dense mat-vecs. Per rebuild: `k`
Jacobian products for the candidate images and `r` base solves for
`inv(P)*C`. A rebuild happens when the base is rebuilt and, lazily, when
the point moves under a frozen base ([`pointmoved!`](@ref)).

# Options

The fields of the [`Floquet`](@ref) value `spec`, validated at its
construction:
...
reason that one does. The three-argument application reads `r` after
writing `z`, so the two must not alias.
"""
...
function FloquetPreconditioner(spec::Floquet, inner::AbstractPreconditioner,
    jvp!, b::AbstractVector{T}; state = nothing) where {T<:AbstractFloat}
    n = length(b)
...
# drop the active basis and its images while keeping the candidate bank,
# the rule a base rebuild applies: the candidates are still directions
# worth compressing against the new base, the blocks built on the old one
# are not
...
strength filter, and the compression to `size`.
