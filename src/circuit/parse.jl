...
# becomes a number is in circuit/values.jl, and the legacy tuple netlist is
# converted to a `Circuit` in circuit/legacy.jl. The end of this file holds
...
function parsecircuitlevel(components, connections, interface)
    ...
    # the pin keys are checked by `parseinterface` and not kept: nothing
    # downstream reads them
    pinendpoints, _ = parseinterface(table, interface)
...
#
# `compile` (circuit/compile.jl) interns the net name of every terminal it
