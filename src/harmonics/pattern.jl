...
# the candidate position of a row in a sorted column, for the insertion
# search of the pattern builders
...
# one work item per column of the complex Jacobian: the count of its stored
# entries, from the node adjacency and the mode coupling
...
# one work item per column: its row indices, in order, at the positions the
# counts reserved
