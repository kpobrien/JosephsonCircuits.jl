...
- `bptr`, `bsrc`, `bcoef`: the segment of frequency domain slots of the
  ...
  times `Lscale/Lj` as the coefficient.
...
- `kptrzero`, `cptrzero`: the row pointers of an empty linear term of each
  form, for applying the Josephson part alone.
- `lptr`, `lwide`: the first real slot of each complex index and whether that
  mode is not self conjugate.
- `forward!`, `backward!`, `forwardcomplex!`, `backwardcomplex!`,
  `realtocomplex!`, `complextoreal!`: the compiled kernels of the two maps
  in each representation and of the conversions between them.
- `backend`, `nslots`, `ncomplex`: the backend, the number of real slots and
  the number of complex indices.
"""
...
incident to its node, scaled by the incidence matrix entry and `Lscale/Lj`,
...
"""
function plannonlinearterm(Rbnm::SparseMatrixCSC, Ljb::SparseVector, Lscale,
    Nbranches::Integer, freqindexmap::Vector{Int},
    conjsourceindices::Vector{Int}, conjtargetindices::Vector{Int},
    phimatrix::AbstractArray, Knm::SparseMatrixCSC, layout::ModeLayout,
    ...
    # strictly below the limit: a pointer array's last entry is the count
    # plus one
    ...
    bptr, bsrc, bcoef = backwardjosephsonmap(Ti, T, Rbnm, Ljb, Lscale, Nmodes,
