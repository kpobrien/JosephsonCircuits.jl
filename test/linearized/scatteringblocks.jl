...
@testset verbose=true "scattering parameter stamps" begin
    ...
    @testset "hybrid coefficient evaluation" begin
        ...
        JosephsonCircuits.evaluatehybrid!(B, Cm, blk, ws,
            JosephsonCircuits.HybridWorkspace())
        ...
        JosephsonCircuits.evaluatehybrid!(B, Cm, short,
            [2*pi*5e9, 0.0, 2*pi*1e9], JosephsonCircuits.HybridWorkspace())
        ...
        JosephsonCircuits.evaluatehybrid!(B2, C2m, att, [2*pi*5e9], JosephsonCircuits.HybridWorkspace())
        @test isapprox(B2[1,2,1], -0.5/sqrt(Z0); rtol = 1e-12)
        @test isapprox(C2m[1,2,1], 0.5*sqrt(Z0); rtol = 1e-12)
        # unequal reference impedances: the stamp must annihilate every
        # voltage and current pair the wave equations b = S a produce, and
        # the diagonal of the impedances multiplies I - S and I + S on the
        # right (the column port), which equal impedances cannot tell
        S = ComplexF64[0.1 0.5im; 0.5im 0.3]
        z = [25.0, 100.0]
        blk2 = ScatteringParameters(S; zref = z)
        JosephsonCircuits.evaluatehybrid!(B2, C2m, blk2, [2*pi*5e9], JosephsonCircuits.HybridWorkspace())
        for a in (ComplexF64[1, 0], ComplexF64[0, 1], ComplexF64[1, im])
            b = S*a
            v = sqrt.(z).*(a + b)
            i = (a - b)./sqrt.(z)
            @test norm(B2[:,:,1]*v - C2m[:,:,1]*i) < 1e-12
        end
        # a direct connection between a 25 ohm port and a 100 ohm port is
        # a wire: v1 = v2 and i1 = -i2 for any drive
        rho = (100 - 25)/(100 + 25)
        Sw = [rho sqrt(1 - rho^2); sqrt(1 - rho^2) -rho]
        wire = ScatteringParameters(Sw; zref = z)
        JosephsonCircuits.evaluatehybrid!(B2, C2m, wire, [2*pi*5e9], JosephsonCircuits.HybridWorkspace())
        for (v, i) in ((ComplexF64[1, 1], ComplexF64[0.3, -0.3]),
                       (ComplexF64[im, im], ComplexF64[-2, 2]))
            @test norm(B2[:,:,1]*v - C2m[:,:,1]*i) < 1e-12
        end
    end

    @testset "a block pair fills its own design parameter slot" begin
        # with only block pairs the sensitivity axis is the design
        # parameters, not the pair count: a pair on the second of two
        # parameters writes the second slot and leaves the first zero
        Z0 = 50.0
        theta = 100e-15
        ...
        dS(w) = (z = 1/(im*w*theta*Z0); d = -2*z/(theta*(z+2)^2);
            [d -d; -d d])
        mk(f) = Circuit([:p1 => Port(1), :cc => ScatteringParameters(f;
                nports = 2, grounded = false), :r2 => Resistor(50.0)],
            [((:p1, 1), (:cc, 1, 1)), ((:cc, 2, 1), (:r2, 1)),
        ...
        out = hblinsolve(ws, psc, cg, Dict{Symbol,Number}(), sf;
            ...
            sensitivityblockpairs = [("cc", 2,
                ScatteringParameters(dS; nports = 2, grounded = false))])
        @test size(out.Ssensitivity) == (1, 1, 2, 2)
        @test all(iszero, out.Ssensitivity[:, :, 1, :])
        # `seriesS` closes over `theta`, so moving it moves the block
        t0 = theta
        h = 1e-6*t0
        Sat(t) = (theta = t; hblinsolve(ws, mk(seriesS); keyedarrays = false).S)
        fd = (Sat(t0 + h) .- Sat(t0 - h))./(2h)
        theta = t0
    ...
    @testset "a block pair names one instance of a shared definition" begin
        ...
        # over the second alone, not the sum over both
        Z0 = 50.0
        series(t) = w -> (z = 1/(im*w*t*Z0);
            [z/(z+2) 2/(z+2); 2/(z+2) z/(z+2)])
        dseries(t) = w -> (z = 1/(im*w*t*Z0); d = -2*z/(t*(z+2)^2);
            [d -d; -d d])
        t0 = 100e-15
        mk(b1, b2) = Circuit([:p1 => Port(1), :cc1 => b1, :cc2 => b2,
                :r2 => Resistor(50.0)],
            [((:p1, 1), (:cc1, 1, 1)), ((:cc1, 2, 1), (:cc2, 1, 1)),
             ((:cc2, 2, 1), (:r2, 1)),
             ((:cc1, 1, 2), (:cc1, 2, 2), (:cc2, 1, 2), (:cc2, 2, 2),
              (:p1, 2), (:r2, 2), Ground)])
        blk(t) = ScatteringParameters(series(t); nports = 2, grounded = false)
        shared = blk(t0)
        ws = 2*pi*[4.0e9, 5.0e9]
        ...
        out = hblinsolve(ws, psc, cg, Dict{Symbol,Number}(), sf;
            ...
            sensitivityblockpairs = [("cc2", 1,
                ScatteringParameters(dseries(t0); nports = 2, grounded = false))])
        h = 1e-6*t0
        S(t2) = hblinsolve(ws, mk(blk(t0), blk(t2)); keyedarrays = false).S
        fd = (S(t0 + h) .- S(t0 - h))./(2h)
        @test isapprox(out.Ssensitivity[:, :, 1, :], fd; rtol = 1e-5)
    end

    @testset "unequal reference impedances through the solve" begin
        # the wire block above between a 50 ohm port and a 75 ohm load is
        # a 75 ohm load seen from 50 ohms: S11 = 0.2. A block with a 75
        # ohm reference on its second port stamps the same wire, and the
        # device path (through the tabulated form) agrees with the host.
        rho = (75 - 50)/(75 + 50)
        Sw = ComplexF64[rho sqrt(1 - rho^2); sqrt(1 - rho^2) -rho]
        ws = 2*pi*(4.0:0.5:6.0)*1e9
        for blk in (ScatteringParameters(Sw; zref = [50.0, 75.0]),
                    ScatteringParameters((collect(ws), repeat(Sw, 1, 1, length(ws)));
                        zref = [50.0, 75.0]))
            c = Circuit([(:p1, "1", "0", Port(1)),
                         (:w, "1", "2", blk),
                         (:r2, "2", "0", Resistor(75.0))])
            out = hblinsolve(ws, c; keyedarrays = false)
            @test all(isapprox.(out.S[1, 1, :], rho; atol = 1e-12))
        end
    end
    ...
    @testset "two port floating block reproduces a series capacitor" begin
        ...
        stamped = Circuit(
            ...
            [((:p1, 1), (:cc, 1, 1)),
             ((:cc, 2, 1), (:jj, 1), (:c2, 1)),
             ((:cc, 1, 2), (:cc, 2, 2), (:jj, 2), (:c2, 2), (:p1, 2), Ground)],
        )
        sol1 = hbsolve(ws, wp, sources, (8,), (16,), lumped)
        sol2 = hbsolve(ws, wp, sources, (8,), (16,), stamped)
    ...
    @testset "scattering blocks inside a JTWPA" begin
        ...
        function twpa(lumped::Bool)
            ...
            components = Any[:p1 => Port(1; Z0 = 50.0),
                :cin => shuntfor(Cg/2)]
            connections = Any[]
            ground = Any[(:p1, 2)]
            ...
            push!(components, :cout => shuntfor(Cg/2),
                :p2 => Port(2; Z0 = 50.0))
            push!(connections, ((prev, 2), (:cout, 1), (:p2, 1)))
            lumped && push!(ground, (:cout, 2))
            push!(ground, (:p2, 2), Ground)
    ...
    @testset "a dissipative block adds its vacuum noise" begin
        ...
        twoportR(x) = Circuit(
            Any[:p1 => Port(1; Z0 = 50.0), :x => x,
                :l1 => Inductor(1000.0e-12), :p2 => Port(2; Z0 = 50.0)],
            Any[((:p1,1), (:x,1)),
                ((:x,2), (:l1,1), (:p2,1)),
                ((:l1,2), (:p1,2), (:p2,2), Ground)])
        twoportB(b) = Circuit(
            Any[:p1 => Port(1; Z0 = 50.0), :x => b,
                :l1 => Inductor(1000.0e-12), :p2 => Port(2; Z0 = 50.0)],
            Any[((:p1,1), (:x,1,1)),
                ((:x,2,1), (:l1,1), (:p2,1)),
    ...
    @testset "block noise channels: which blocks carry them and how they are named" begin
        ...
        # a table whose knots are unitary is lossless only where its
        # interpolant is: a phase which rotates between coarse knots
        # interpolates inside the unit circle (a chord), a constant phase
        # does not
        unit = reshape([complex(cis(w*1e-11)) for w in ftab], 1, 1, :)
        @test !JosephsonCircuits.provablylossless(
            ScatteringParameters((ftab, unit); nports = 1, grounded = true))
        flat = reshape(fill(complex(cis(0.3)), length(ftab)), 1, 1, :)
        @test JosephsonCircuits.provablylossless(
            ScatteringParameters((ftab, flat); nports = 1, grounded = true))
        @test !JosephsonCircuits.provablylossless(
            ScatteringParameters((ftab, 0.9*flat); nports = 1, grounded = true))
        @test !JosephsonCircuits.provablylossless(
    ...
    @testset "blocks with more than two ports" begin
        ...
        fourport(S) = Circuit(
            Any[:p1 => Port(1; Z0 = 50.0), :p2 => Port(2; Z0 = 50.0), :l1 => Inductor(1000.0e-12),
            ...
            Any[((:p1,1), (:x,1)),
                ...
                ((:x,3), (:ct,1)),
                ((:x,4), (:rt,1)),
                ((:l1,2), (:ct,2), (:rt,2), (:p1,2), (:p2,2), Ground)])
    ...
    @testset "sensitivities of a circuit containing a scattering block" begin
        ...
        Z0 = 50.0
        blockjpa(Cc) = Circuit(
            Any[:p1 => Port(1; Z0 = Z0), :cc => Capacitor(Cc),
                :jj => JosephsonJunction(1000.0e-12),
    ...
    @testset "the added noise covariance" begin
        ...
        mkc(bT) = Circuit(
            Any[:p1 => Port(1; Z0 = Z0),
                :x => ScatteringParameters(ComplexF64[0.2 0.6; 0.6 0.2];
                    grounded = false,
                    noise = isnothing(bT) ? Passive() : ThermalEquilibrium(bT)),
                :l1 => Inductor(1000.0e-12), :p2 => Port(2; Z0 = Z0), :cl => Capacitor(200.0e-15),
                :rt => Resistor(2.0e4)],
            Any[((:p1,1), (:x,1,1)),
                ((:x,2,1), (:l1,1), (:p2,1), (:cl,1)),
                ((:cl,2), (:rt,1)),
                ((:x,1,2), (:x,2,2), (:l1,2), (:p1,2), (:p2,2), (:rt,2), Ground)])
        ...
        two = hblinsolve(vcat(wsc, 1.05 .* wsc), mkc(nothing);
            returnCnoise = true)
        @test size(two.Cnoise) == (1, 2, 1, 2, 2)
        @test isapprox(reshape(Array(two.Cnoise)[:, :, :, :, 1], 2, 2),
    ...
    @testset "one block definition used as several instances" begin
        ...
        mk(shared) = begin
            ...
            Circuit(
                Any[:p1 => Port(1; Z0 = 50.0), :cc => Capacitor(100e-15),
                    :b1 => b1, :l => Inductor(1e-9), :b2 => b2,
                    :jj => JosephsonJunction(1e-9), :gnd => Ground()],
                Any[[(:p1,1),(:cc,1)], [(:cc,2),(:b1,1),(:l,1)],
                    [(:l,2),(:b2,1),(:jj,1)],
                    [(:p1,2),(:jj,2),(:gnd,1)]])
        end
    ...
    @testset "stamp system from the compiled blocks" begin
        ...
        c = Circuit(
            Any[:p1 => Port(1; Z0 = 50.0),
                ...
                :jj => JosephsonJunction(1e-9), :gnd => Ground()],
            Any[[(:p1,1),(:b1,1),(:t,1,1)], [(:t,2,1),(:l,1)],
                [(:l,2),(:b2,1),(:jj,1)],
                [(:p1,2),(:jj,2),(:t,1,2),(:t,2,2),(:gnd,1)]])
        cc = JC.compile(c)
        # three instances, two of them the same definition, and none of them
        # in the flat table
        @test [b.path for b in cc.scatteringblocks] == ["b1", "b2", "t"]
        @test cc.scatteringblocks[1].definition === one
        @test cc.scatteringblocks[2].definition === one
        @test !any(==(:S), cc.componenttypes)
        @test JC.countscatteringports(cc) == 4
        @test !isempty(cc.scatteringblocks)
        ...
        for Nmodes in (1, 2, 4)
            ...
            @test [x.auxbase for x in a.blocks] ==
                [7, 7 + Nmodes, 7 + 2*Nmodes]
            # two instances of one definition are at their own terminals
            @test a.blocks[1].signalnodes != a.blocks[2].signalnodes
...
@testset "the loss an interpolant introduces has a noise channel" begin
    # unitary knots with a lossy interpolant between them: at the
    # midpoint the one port absorbs everything, and its noise channel has
    # to make up the whole commutation relation. Declaring it lossless is
    # refused, since the table cannot prove it.
    table = ScatteringParameters(([2*pi*1e9, 2*pi*3e9],
        reshape(ComplexF64[1, -1], 1, 1, 2)); nports = 1, grounded = true)
    c = Circuit([(:p1, "1", "0", Port(1)), (:s, "1", table)])
    out = hblinsolve([2*pi*2e9, 2*pi*1.5e9], c; keyedarrays = false,
        returnSnoise = true, returnCM = true)
    @test abs(out.S[1, 1, 1]) < 1e-12        # the midpoint absorbs everything
    @test size(out.Snoise, 1) == 1             # one channel, the block's
    @test isapprox(out.CM[1, 1], 1.0; atol = 1e-12)
    @test isapprox(out.CM[1, 2], 1.0; atol = 1e-12)
