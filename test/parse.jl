...
@testset verbose=true "input path" begin
    ...
    @testset "netlist form" begin
        ...
        # identifiers of different types with one name are one component
        # further down, so they are refused in both forms
        @test_throws ArgumentError Circuit(
            Any[:r1 => Resistor(50.0), "r1" => Resistor(50.0), :p1 => Port(1)],
            Any[[(:p1,1),(:r1,1),("r1",1)], [(:p1,2),(:r1,2),("r1",2),Ground]])
        @test_throws ArgumentError compile(Circuit([
            (:r1, 1, 0, Resistor(50.0)), ("r1", 1, 0, Resistor(50.0)),
            (:p1, 1, 0, Port(1))]))
