...
@testset verbose=true "hbsystem" begin
    ...
    @testset "both source vectors are on the backend" begin
        ...
        mk = bnm -> JosephsonCircuits.HBSystem(d.sys.Rbnm,
            ...
            d.sys.wmodes2m, bnm, d.sys.Ljb, d.sys.Ljbm, d.sys.Lscale,
    ...
    @testset "the cosine coefficients survive a product, and a transform its input" begin
        circuit, defs = testjpacircuitnumeric()
        d = JosephsonCircuits.hbnlsolve((2*pi*4.75001e9,), (8,),
            [(mode=(1,),port=1,current=0.02e-6)], circuit, defs;
            returnsystem = true)
        sys = d.sys
        x = 0.05 .* randn(length(d.xr))
        JosephsonCircuits.setpoint!(sys, x)
        c1 = copy(JosephsonCircuits.cosphimatrix(sys))
        # a product uses `phimatrix` as scratch; the cosine buffer is not it
        v = randn(length(x)); Jv = similar(v)
        JosephsonCircuits.jacobianvectorproduct!(Jv, sys, v)
        F = similar(v); JosephsonCircuits.residual!(F, sys)
        @test JosephsonCircuits.cosphimatrix(sys) == c1
        # the forward transform reads the cached time domain sine as it is
        s1 = copy(sys.sintd)
        JosephsonCircuits.residual!(F, sys)
        @test sys.sintd == s1
        # a new point invalidates the buffer
        JosephsonCircuits.setpoint!(sys, 2 .* x)
        @test JosephsonCircuits.cosphimatrix(sys) != c1
    end
    ...
    @testset "jacobian! without a plan throws" begin
        ...
        sys = JosephsonCircuits.HBSystem(d.sys.Rbnm,
            d.sys.invLnm, d.sys.Gnm, d.sys.Cnm, d.sys.wmodesm,
            d.sys.wmodes2m, d.sys.bnm, d.sys.Ljb, d.sys.Ljbm, d.sys.Lscale,
            d.Nbranches, d.sys.freqindexmap, d.sys.conjsourceindices,
            d.sys.conjtargetindices, copy(d.sys.phimatrix),
            copy(d.sys.worktd), d.sys.irfftplan, d.sys.rfftplan,
            d.sys.modelayout, nothing, nothing)
